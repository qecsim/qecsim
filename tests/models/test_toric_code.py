...
def test_toric_lattice_distance(size, a_index, b_index, expected):
    assert ToricCode(*size).distance(a_index, b_index) == expected


@pytest.mark.parametrize('size, a_index, b_index', [
    ((5, 5), (0, 1, 1), (1, 2, 2)),  # different lattices
])
def test_toric_lattice_invalid_distance(size, a_index, b_index):
    code = ToricCode(*size)
    with pytest.raises(IndexError):
