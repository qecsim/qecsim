...
def test_planar_lattice_distance(size, a_index, b_index, expected):
    assert PlanarCode(*size).distance(a_index, b_index) == expected
...
def test_planar_lattice_invalid_distance(size, a_index, b_index):
    code = PlanarCode(*size)
    with pytest.raises(IndexError):
        code.distance(a_index, b_index)
