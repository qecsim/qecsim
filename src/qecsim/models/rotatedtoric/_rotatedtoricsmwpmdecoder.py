...
class RotatedToricSMWPMDecoder(Decoder, DecoderFTP):
    ...
    def __init__(self, itp=False, eta=None):
        ...
        try:  # paranoid checking for CLI.
            if not (eta is None or (eta > 0.0 and math.isfinite(eta))):
                raise ValueError('RotatedToricSMWPMDecoder valid eta values are None or number > 0.0')
        except TypeError as ex:
            raise TypeError('RotatedToricSMWPMDecoder invalid parameter type') from ex
        self._itp = itp
        self._eta = eta
    ...
    def decode_ftp(self, code, time_steps, syndrome, error_model=BitPhaseFlipErrorModel(), error_probability=0.1,
        ...
        # SYMMETRY MATCHING
        # prepare graphs
        graphs = _graphs(code, time_steps, syndrome, error_probability, measurement_error_probability, bias)
        # minimum weight matching
        matches = _matching(graphs)
        ...
        symmetry_recovery, symmetry_recovery_x_tp, symmetry_recovery_z_tp = _recovery_tparities(
        ...
        cluster_recovery, cluster_recovery_x_tp, cluster_recovery_z_tp = _cluster_recovery_tparities(
        ...
        if self._itp or time_steps == 1:
            ...
            if not step_measurement_errors:
                raise QecsimException('Failed to test t-parity. step_measurement_errors not provided.')
            # extract t-parity for measurement errors
            measurement_error_tps = _measurement_error_tparities(code, step_measurement_errors[-1])
...
def _graphs(code, time_steps, syndrome, error_probability=None, measurement_error_probability=None, eta=None):
    ...
    * Edges are added between nodes on a given row or column; such edges are weighted by the distance function.
    ...
    :param eta: Bias (a positive finite number or None for infinite bias), i.e. p_y / (p_x + p_z).
    :type eta: float or None
    :return: List of graphs weighted edges between plaquette nodes, consistent with the syndrome,
...
@functools.lru_cache()
def _tparity(time_steps, a_t, b_t):
...
def _recovery_tparities(code, time_steps, clusters):
    ...
    for cluster in clusters:
        ...
        for (a_t, a_x, a_y), (b_t, b_x, b_y) in zip(x_path[::2], x_path[1::2]):
            ...
            x_tparity ^= _tparity(time_steps, a_t, b_t)
        ...
        for (a_t, a_x, a_y), (b_t, b_x, b_y) in zip(z_path[::2], z_path[1::2]):
            operator ^= code.new_pauli().path((a_x, a_y), (b_x, b_y)).to_bsf()
            z_tparity ^= _tparity(time_steps, a_t, b_t)
    return operator, x_tparity, z_tparity


def _measurement_error_tparities(code, measurement_error):
...
def _cluster_distance(code, time_steps, a_node, b_node):
    ...
    * Distance is defined as shortest Manhattan distance between any pair of plaquette indices in the
      clusters, noting that all dimensions are periodic.
...
def _cluster_graph(code, time_steps, clusters):
    ...
    * Edges are added between all nodes with weight given by the cluster_distance function.
    ...
    for a_node, b_node in itertools.combinations(defective_cluster_nodes + neutral_cluster_nodes, 2):
        graph.add_edge(a_node, b_node, _cluster_distance(code, time_steps, a_node, b_node))
    return graph


def _cluster_recovery_tparities(code, time_steps, matches):
    """Operator consisting of a paths of Pauli operators to fuse clusters together with t-parity for X and Z plaquette
    types.
